...
def test_product_named_in_another_language_still_resolves_by_program_id():
    ...
    assert sr.select({"program_id": "no_such", "type": "K-ETA"}, "KOR")["id"] == "kor_keta"
...
def test_parsed_list_must_not_omit_eligible_countries_present_in_quote(registry):
    e = entry("kor_keta")
    registry([dict(e, eligible_nationalities=[n for n in e["eligible_nationalities"] if n != "JPN"])])
    assert sr.for_destination("KOR") == []
    assert any("omits country items" in error and "JPN" in error
               for error in sr.store_status()["errors"][0]["errors"])
