...
def test_grading_of_the_route_answer_asks_for_the_verdict_check_and_completeness():
    # Their ladder: one official source checked for the verdict, every
    # required cell filled or documented, no conflict. The route's own answer
    # is graded on that; the per-field proof gate stays on product rows
    # (test_product_row_keeps_the_per_field_proof_gate), where a product's
    # terms can be inherited from a page that never described the product.
    assert record(prov=dict(PROV, verifier='ai'))['confidence_level'] == 'High'
    assert record(prov=dict(PROV, fields=['disposition']))['confidence_level'] == 'High'
    ...
    assert record(prov=None, grounded_ok=True, collected_at='2026-09-09',
                  grounded_fields=['disposition'])['confidence_level'] == 'High'
    assert record(g={k: v for k, v in FREE.items() if not k.startswith("permitted_stay")},
                  prov=dict(PROV, fields=['disposition']))['confidence_level'] == 'Medium'
...
def test_binary_label_does_not_change_existing_publication_evidence_gate():
    # A source-backed verdict with no verification of stay: the display
    # grade follows completeness, and the hold gate is untouched.
    incomplete_review = record(prov=dict(PROV, fields=['disposition']))
    assert incomplete_review['confidence_level'] == 'High'
