"""guard-20260912 T7: the authorisation-page rule is symmetric.

serve_time_invariants used to test the cited page only when the verdict was
VISA_REQUIRED, so an unconditional exemption resting on the K-ETA portal
itself passed every invariant. Ten live Korea rows were served that way,
which is Trip.com's first finding caught as a class rather than for one
nationality."""
...
def test_unconditional_exemption_on_a_scheme_page_contradicts():
    g = {"disposition": "VISA_EXEMPT", "requirement_detail": "unconditional_visa_free",
         "source_url": KETA_PORTAL}
    assert EXEMPT_ON_SCHEME in kp.serve_time_invariants(g)
    # The same shape with no detail at all, and with the page only as the portal.
    assert EXEMPT_ON_SCHEME in kp.serve_time_invariants(dict(g, requirement_detail=None))
    assert EXEMPT_ON_SCHEME in kp.serve_time_invariants(
...
def test_all_ten_korea_rows_are_caught(shipped, nat):
    ...
    K-ETA portal. The contradiction holds the row until it is corrected."""
...
def test_a_scheme_whose_list_is_not_established_withholds_nothing(tmp_path, monkeypatch):
    ...
    the local copy) is reported by the sweep and never withheld. Once a list
    is established the same page contradicts."""
    import json
    ...
    try:
        ...
        assert EXEMPT_ON_SCHEME in kp.serve_time_invariants(esta)
        assert EXEMPT_ON_SCHEME not in kp.serve_time_invariants(etias)
    finally:
