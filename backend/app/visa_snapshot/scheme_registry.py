...
def _entry_errors(e: dict, today: date) -> list[str]:
    ...
    omitted = codes - {str(n).upper() for n in nats}
    if omitted:
        errors.append("eligible_nationalities omits country items from list_quote: "
                      + ", ".join(sorted(omitted)))
    return errors
...
def identity_conflicts(item: dict, destination: str) -> list[str]:
    """A recognized id cannot mask a different, unambiguous product identity.

    A generic ministry landing page and a label matching multiple schemes
    establish no alternative identity. Do not select a programme from those.
    """
    if not isinstance(item, dict):
        return []
    candidates = for_destination(destination)
    program_id = str(item.get("program_id") or "").strip()
    explicit = next((e for e in candidates if e["id"] == program_id), None)
    if explicit is None:
        return []
    portal = str(item.get("official_portal_url") or "").lower()
    portal_matches = [e for e in candidates if e.get("portal_marker")
                      and str(e["portal_marker"]).lower() in portal]
    name = " ".join(str(item.get(k) or "") for k in ("type", "visa_category", "visa_label", "name"))
    name_matches = [e for e in candidates if any(re.search(p, name, re.I) for p in e.get("patterns") or [])]
    conflicts = []
    for label, matches in (("application portal", portal_matches), ("product name", name_matches)):
        if len(matches) == 1 and matches[0]["id"] != explicit["id"]:
            conflicts.append(f"product identity: program_id {explicit['id']} conflicts with the "
                             f"{label} identifying {matches[0]['id']}")
    return conflicts
...
def select(item: dict, destination: str, *, headline: bool = False) -> dict | None:
    ...
    if explicit:
        match = next((e for e in candidates if e["id"] == explicit), None)
        if match is not None:
            return match
        # An unknown or other-destination id must not disable the known
        # portal/name/detail checks (for example THA -> AUS ETA 601).
