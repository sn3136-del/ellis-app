...
time and edit time inherit the guard with no new wiring. The rules:
  ...
  3. list membership establishes eligibility to apply, not an obligation
     to apply. Visa-free entry, temporary authorization waivers and optional
     longer-stay visas can coexist with an eligible nationality list. The
     route's supported policy, not this negative eligibility guard, decides
...
def _evaluate(guidance: dict, route: dict) -> tuple[list[str], list[str]]:
    ...
    for item, headline in [(guidance, True)] + [(p, False) for p in products]:
        out.extend(scheme_registry.identity_conflicts(item, destination))
    ...
    # A scheme list is not a visa-requirement table. In particular, a
    # nationality can qualify for a visa and separately qualify for an
    # exemption. Missing scheme research remains visible to the sweep, but
    # membership alone must never manufacture a contradiction or hold.
