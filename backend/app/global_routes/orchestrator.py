...
def _unpark_for_rebuild(db, req) -> None:
    ...
    # A rebuild exists to apply what changed since the last attempt, and the
    # SPECIFICATION is where most of that lives — how the flow is shaped from
    # the observed structure. Keeping the old spec_id made the build reuse a
    # spec generated by the previous code, so a specgen fix could be correct,
    # committed and tested and STILL not reach the portal it fixed (Indonesia
    # kept failing on the invented '#submit-btn' for exactly this reason).
    # Recon evidence is untouched; only the derived spec is dropped.
    ev = dict(req.portal_evidence or {})
    if ev.pop("spec_id", None) is not None:
        req.portal_evidence = ev
    db.commit()
