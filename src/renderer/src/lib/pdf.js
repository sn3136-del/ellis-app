...
const CSS = `
  * { box-sizing: border-box; }
  body { font-family: -apple-system, "Helvetica Neue", Arial, sans-serif; color: #0a0a0a; margin: 0; padding: 48px 54px; }
  .hd { display: flex; justify-content: space-between; align-items: flex-end; border-bottom: 2px solid #0a0a0a; padding-bottom: 14px; margin-bottom: 24px; }
  .hd .brand { font-size: 26px; font-weight: 800; letter-spacing: -0.5px; }
  .hd .org { font-size: 12px; color: #555; text-align: right; }
  h1 { font-size: 20px; margin: 0 0 4px; }
  .sub { color: #555; font-size: 13px; margin-bottom: 22px; }
  .sec { font-size: 11px; letter-spacing: 1.5px; text-transform: uppercase; color: #555; margin: 22px 0 10px; font-weight: 700; }
  table { width: 100%; border-collapse: collapse; }
  td { padding: 8px 6px; border-bottom: 1px solid #e6e6e6; font-size: 13px; vertical-align: top; }
  td.k { width: 230px; color: #555; font-weight: 600; }
  .miss { color: #0a0a0a; font-weight: 700; background: #f0f0f0; padding: 1px 6px; border-radius: 4px; }
  .chip { display: inline-block; border: 1px solid #ccc; border-radius: 999px; padding: 2px 10px; font-size: 11px; margin-right: 6px; }
  ul { margin: 6px 0; padding-left: 20px; } li { font-size: 13px; margin-bottom: 4px; }
  .foot { margin-top: 40px; padding-top: 14px; border-top: 1px solid #e6e6e6; font-size: 11px; color: #888; }
`

function shell(title, org, body) {
  const date = new Date().toLocaleDateString(undefined, { year: 'numeric', month: 'long', day: 'numeric' })
  return `<!doctype html><html><head><meta charset="utf-8"><style>${CSS}</style></head><body>
    <div class="hd"><div class="brand">Ellis</div><div class="org">${esc(org)}<br>Generated ${date}</div></div>
    ${body}
    <div class="foot">Prepared by Ellis — AI workspace for immigration operations. Operational document; licensed counsel reviews and approves the final filing. Not legal advice.</div>
  </body></html>`
}

function esc(s) { return String(s ?? '').replace(/[&<>]/g, (c) => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;' }[c])) }
function rows(fields) { return (fields || []).map((f) => `<tr><td class="k">${esc(f.label)}</td><td>${f.value === 'MISSING' ? '<span class="miss">MISSING</span>' : esc(f.value)}</td></tr>`).join('') }
function list(items) { return `<ul>${(items || []).map((i) => `<li>${esc(typeof i === 'string' ? i : (i.item || i.action || i.label))}</li>`).join('')}</ul>` }

async function org() { const s = await ellis.getSettings(); return s.organizationName || 'Ellis Immigration' }

/* ---------------- Official government form rendering ---------------- */
const FORM_CSS = `
  * { box-sizing: border-box; }
...
function fv(c, ...keys) { const f = c.facts || {}; for (const k of keys) { if (f[k] && String(f[k]).trim()) return f[k] } return '' }
function splitName(c) {
  const n = (c.applicantName || '').trim()
  const p = n.split(/\s+/)
  if (p.length < 2) return { last: n, first: '' }
  return { last: p[p.length - 1], first: p.slice(0, -1).join(' ') }
}
function formKey(name) {
  const s = (name || '').toLowerCase()
  if (/i-?129/.test(s)) return 'i129'
  if (/i-?539/.test(s)) return 'i539'
  if (/i-?765/.test(s)) return 'i765'
  if (/i-?20/.test(s)) return 'i20'
  if (/ds-?160/.test(s)) return 'ds160'
  if (/1295/.test(s)) return 'imm1295'
  if (/1294/.test(s)) return 'imm1294'
  if (/5257/.test(s)) return 'imm5257'
  return 'generic'
}

// Build an authentic-looking, filled government form for the case.
function buildForm(data, c) {
  const key = formKey(data.formName)
  const { last, first } = splitName(c)
  const dob = fv(c, 'Date of Birth', 'DOB', 'Birth Date')
  const passport = fv(c, 'Passport No.', 'Passport Number', 'Passport')
  const passExp = fv(c, 'Passport Expiry', 'Passport Valid Until', 'Passport Expiration')
  const i94 = fv(c, 'I-94', 'I-94 Number', 'Most Recent I-94 Number')
  const statusUntil = fv(c, 'Status Valid Until', 'Authorized Stay Until', 'Valid Until')
  const usAddr = fv(c, 'U.S. Address', 'US Address', 'Address in the United States', 'Address')
  const caAddr = fv(c, 'Canadian Address', 'Address in Canada', 'Address')
  const wage = fv(c, 'Offered Wage', 'Wage', 'Annual Salary', 'Salary')
  const jobTitle = fv(c, 'Job Title', 'Position', 'Occupation')
  const program = fv(c, 'Program', 'Program of Study', 'Course', 'Major')
  const sevis = fv(c, 'SEVIS ID', 'SEVIS')
  const funds = fv(c, 'Proof of Funds', 'Funds', 'Financial Support', 'Available Funds')
  const employer = c.employer || fv(c, 'Employer', 'School', 'Designated Learning Institution', 'DLI')
  const vt = (c.visaType || '').toLowerCase()
  const applicant = [
    field('Family Name (Last Name)', last),
    field('Given Name (First Name)', first),
    field('Date of Birth (mm/dd/yyyy)', dob),
    field('Country of Birth', c.originCountry),
    field('Country of Citizenship/Nationality', c.originCountry),
    field('Passport / Travel Document Number', passport),
    field('Passport Expiration Date', passExp),
    field('Sex', fv(c, 'Sex', 'Gender'))
  ]

  if (key === 'i129') {
    return {
      dept: 'U.S. Department of Homeland Security', bureau: 'U.S. Citizenship and Immigration Services',
      fno: 'Form I-129', ftitle: 'Petition for a Nonimmigrant Worker', omb: 'OMB No. 1615-0009 · Expires 12/31/2026',
      parts: [
        part('Part 1. Petitioner Information', [
          fullField('Legal Name of Petitioner (Employer)', employer),
          field('Contact / Authorized Signatory', fv(c, 'Signatory', 'HR Contact')),
          field('Federal Employer ID Number (FEIN)', fv(c, 'FEIN', 'EIN')),
          fullField('Petitioner Mailing Address', fv(c, 'Employer Address', 'Company Address'))
        ]),
        checkPart('Part 2. Information About This Petition (Requested Action)', [
          { label: 'a. Notify the office in Part 4 so the beneficiary can obtain a visa or be admitted.', on: true },
          { label: 'b. Change the status and extend the stay of the beneficiary (already in the U.S.).', on: !!i94 },
          { label: 'c. Extend the stay of the beneficiary in current status.', on: false }
        ]),
        checkPart('Part 2 (cont.). Requested Nonimmigrant Classification', [
          { label: 'H-1B — Specialty Occupation', on: vt.includes('h-1b') || vt.includes('h1b') },
          { label: 'L-1 — Intracompany Transferee', on: vt.includes('l-1') || vt.includes('l1') },
          { label: 'O-1 — Extraordinary Ability', on: vt.includes('o-1') || vt.includes('o1') },
          { label: 'TN — USMCA Professional', on: vt.includes('tn') }
        ]),
        part('Part 3. Beneficiary Information', [
          ...applicant,
          field('U.S. Social Security Number (if any)', fv(c, 'SSN')),
          field('Most Recent I-94 Number', i94),
          field('Current Nonimmigrant Status', fv(c, 'Current Status', 'Status')),
          field('Status Valid Until', statusUntil)
        ]),
        part('Part 4. Employment & Worksite', [
          field('Job Title', jobTitle),
          field('Offered Wage', wage),
          fullField('Worksite Address (U.S.)', usAddr),
          field('Requested Validity From', fv(c, 'Start Date', 'Validity From')),
          field('Requested Validity To', statusUntil)
        ])
      ],
      cert: 'I certify, under penalty of perjury, that this petition and the evidence submitted with it are all true and correct. Authorized signature of petitioner required before filing.'
    }
  }
  if (key === 'i539') {
    return {
      dept: 'U.S. Department of Homeland Security', bureau: 'U.S. Citizenship and Immigration Services',
      fno: 'Form I-539', ftitle: 'Application To Extend/Change Nonimmigrant Status', omb: 'OMB No. 1615-0003 · Expires 09/30/2026',
      parts: [
        part('Part 1. Information About You', [...applicant, field('U.S. Mailing Address', usAddr), field('A-Number (if any)', fv(c, 'A-Number'))]),
        checkPart('Part 2. Application Type', [
          { label: 'a. Extension of stay in current status', on: !vt },
          { label: 'b. Change of status — new status requested below', on: !!vt },
          { label: `Requested status: ${c.visaType || c.pathway}`, on: true }
        ]),
        part('Part 3. Processing Information', [
          field('Current Nonimmigrant Status', fv(c, 'Current Status', 'Status')),
          field('Most Recent I-94 Number', i94),
          field('Current Status Expires On', statusUntil),
          field('Requested Extension Until', fv(c, 'Requested Until', 'Extend Until'))
        ])
      ],
      cert: 'I certify, under penalty of perjury, that this application and the evidence submitted with it are all true and correct.'
    }
  }
  if (key === 'i765') {
    return {
      dept: 'U.S. Department of Homeland Security', bureau: 'U.S. Citizenship and Immigration Services',
      fno: 'Form I-765', ftitle: 'Application for Employment Authorization', omb: 'OMB No. 1615-0040 · Expires 08/31/2026',
      parts: [
        part('Part 1. Reason for Applying', [fullField('Reason', 'Initial permission to accept employment')]),
        part('Part 2. Information About You', [...applicant, field('U.S. Mailing Address', usAddr), field('SEVIS Number (if any)', sevis)]),
        part('Part 2 (cont.). Eligibility', [
          field('Eligibility Category', fv(c, 'Eligibility Category') || (c.pathway === 'student' ? '(c)(3)(B) — STEM OPT' : '(c)(9) — Pending adjustment')),
          field('Most Recent I-94 Number', i94)
        ])
      ],
      cert: 'I certify, under penalty of perjury, that the foregoing is true and correct.'
    }
  }
  if (key === 'i20' || (key === 'generic' && c.pathway === 'student' && c.destinationCountry !== 'Canada')) {
    return {
      dept: 'U.S. Department of Homeland Security', bureau: 'U.S. Immigration and Customs Enforcement — SEVP',
      fno: 'Form I-20', ftitle: 'Certificate of Eligibility for Nonimmigrant Student Status (F-1)', omb: 'OMB No. 1653-0038',
      parts: [
        part('Student Information', [...applicant, field('SEVIS ID Number', sevis)]),
        part('School Information', [
          fullField('School Name (SEVP-certified)', employer),
          field('Program of Study / Major', program),
          field('Program Start Date', fv(c, 'Program Start', 'Start Date')),
          field('Program End Date', fv(c, 'Program End', 'End Date'))
        ]),
        part('Financials', [
          field('Estimated Tuition & Fees', fv(c, 'Tuition')),
          field('Living Expenses', fv(c, 'Living Expenses')),
          fullField('Evidence of Financial Support', funds)
        ])
      ],
      cert: 'This certificate is completed by the Designated School Official (DSO) and used to support the F-1 visa application (DS-160) and SEVIS I-901 fee payment.'
    }
  }
  if (key === 'ds160' || (key === 'generic' && c.destinationCountry !== 'Canada' && c.pathway === 'travel')) {
    return {
      dept: 'U.S. Department of State', bureau: 'Bureau of Consular Affairs',
      fno: 'Form DS-160', ftitle: 'Online Nonimmigrant Visa Application', omb: 'OMB No. 1405-0182',
      parts: [
        part('Personal Information', applicant),
        part('Travel Information', [
          field('Purpose of Trip', fv(c, 'Purpose', 'Purpose of Trip') || (c.pathway === 'travel' ? 'Tourism / Visit (B-2)' : c.visaType || c.pathway)),
          field('Intended Date of Arrival', fv(c, 'Arrival Date', 'Departure')),
          field('Intended Length of Stay', fv(c, 'Length of Stay')),
          fullField('Address Where You Will Stay in the U.S.', usAddr)
        ]),
        part('Passport & Contact', [
          field('Passport Number', passport),
          field('Passport Expiration Date', passExp),
          field('Email Address', fv(c, 'Email')),
          field('Phone Number', fv(c, 'Phone'))
        ])
      ],
      cert: 'I certify that I have read and understood the questions in this application and that my answers are true and correct to the best of my knowledge and belief.'
    }
  }
  if (key === 'imm1295') {
    return { ca: true, dept: 'Immigration, Refugees and Citizenship Canada', bureau: 'PROTECTED WHEN COMPLETED — B',
      fno: 'IMM 1295', ftitle: 'Application for Work Permit Made Outside of Canada', omb: 'IRCC (formerly CIC)',
      parts: [
        part('Personal Details', [...applicant, field('Current Country of Residence', c.originCountry), field('Marital Status', fv(c, 'Marital Status'))]),
        part('Details of Intended Work in Canada', [
          fullField('Name of Employer', employer),
          field('Job Title / Occupation', jobTitle),
          field('LMIA / Offer of Employment Number', fv(c, 'LMIA', 'LMIA Number', 'Offer of Employment Number')),
          fullField('Address of Employer in Canada', fv(c, 'Employer Address', 'Work Location') || caAddr),
          field('Duration of Expected Employment', fv(c, 'Duration', 'Contract Length')),
          field('Wage', wage)
        ]),
        part('Coming Into Canada', [
          field('From (date)', fv(c, 'Start Date', 'Intended Arrival')),
          field('To (date)', fv(c, 'End Date')),
          fullField('Funds Available for Stay (CAD)', funds)
        ])
      ],
      cert: 'I declare that the information I have given is truthful, complete and correct. Application supported by counsel review prior to submission to IRCC.'
    }
  }
  if (key === 'imm1294') {
    return { ca: true, dept: 'Immigration, Refugees and Citizenship Canada', bureau: 'PROTECTED WHEN COMPLETED — B',
      fno: 'IMM 1294', ftitle: 'Application for Study Permit Made Outside of Canada', omb: 'IRCC (formerly CIC)',
      parts: [
        part('Personal Details', [...applicant, field('Current Country of Residence', c.originCountry), field('Marital Status', fv(c, 'Marital Status'))]),
        part('Details of Intended Study in Canada', [
          fullField('Designated Learning Institution (DLI)', employer),
          field('DLI Number', fv(c, 'DLI Number', 'DLI')),
          field('Field / Program of Study', program),
          field('Level of Study', fv(c, 'Level of Study')),
          field('Tuition Cost (CAD/yr)', fv(c, 'Tuition')),
          field('Expected Start Date', fv(c, 'Start Date', 'Program Start'))
        ]),
        part('Means of Support', [
          fullField('Funds Available for Stay (CAD)', funds),
          field('Tuition Paid', fv(c, 'Tuition Paid')),
          field('Living Expenses (GIC)', fv(c, 'GIC', 'Living Expenses'))
        ])
      ],
      cert: 'I declare that the information I have given is truthful, complete and correct. Study permit application reviewed by counsel before submission to IRCC.'
    }
  }
  if (key === 'imm5257') {
    return { ca: true, dept: 'Immigration, Refugees and Citizenship Canada', bureau: 'PROTECTED WHEN COMPLETED — B',
      fno: 'IMM 5257', ftitle: 'Application for Visitor Visa (Temporary Resident Visa)', omb: 'IRCC (formerly CIC)',
      parts: [
        part('Personal Details', [...applicant, field('Current Country of Residence', c.originCountry), field('Marital Status', fv(c, 'Marital Status'))]),
        part('Details of Visit to Canada', [
          field('Purpose of Visit', fv(c, 'Purpose', 'Purpose of Visit') || 'Tourism / Visit family'),
          field('From (date)', fv(c, 'Arrival Date', 'Start Date')),
          field('To (date)', fv(c, 'End Date', 'Return Date')),
          fullField('Address / Host in Canada', caAddr || fv(c, 'Host', 'Host Address'))
        ]),
        part('Means of Support', [fullField('Funds Available for Stay (CAD)', funds)])
      ],
      cert: 'I declare that the information I have given is truthful, complete and correct. Visitor visa application reviewed by counsel before submission to IRCC.'
    }
  }
  // Generic fallback — render from the engine's prepared sections.
  return {
    ca: c.destinationCountry === 'Canada',
    dept: c.destinationCountry === 'Canada' ? 'Immigration, Refugees and Citizenship Canada' : 'U.S. Department of Homeland Security',
    bureau: c.destinationCountry === 'Canada' ? 'PROTECTED WHEN COMPLETED — B' : 'U.S. Citizenship and Immigration Services',
    fno: data.formName, ftitle: data.purpose || 'Immigration Form', omb: data.jurisdiction || '',
    parts: (data.sections || []).map((s) => part(s.title, (s.fields || []).map((x) => field(x.label, x.value)))),
    cert: 'Prepared by Ellis from the case file. Licensed counsel reviews and approves before filing.'
  }
}

function formHtml(data, c, orgName) {
  const spec = buildForm(data, c)
    ...
    <div class="gov">
      <div class="govhead">
        <div class="agency">
          ...
          <div class="ellis">Auto-completed by Ellis for ${esc(orgName)} · ${date}</div>
        </div>
        <div class="formmeta">
          <div class="fno">${esc(spec.fno)}</div>
          <div class="ftitle">${esc(spec.ftitle)}</div>
          <div class="omb">${esc(spec.omb)}</div>
        </div>
      </div>
      <div class="usconly">${spec.ca ? 'FOR IRCC USE ONLY' : 'For USCIS Use Only'} — Draft generated by Ellis. Review and sign before submission.</div>
      ${spec.parts.join('')}
      <div class="cert">
        ${esc(spec.cert)}
        <div class="sigrow"><div class="s">Signature</div><div class="s">Date (mm/dd/yyyy)</div></div>
      </div>
    </div>
    ${missing.length ? `<div class="miss-summary"><b>${missing.length} field(s) still required before filing:</b> ${esc(missing.join(', '))}</div>` : '<div class="miss-summary"><b>All required fields completed</b> from the case file — ready for counsel review.</div>'}
    <div class="notebar">Prepared by Ellis — AI workspace for immigration operations. This is an operational draft populated from the case file; a licensed attorney reviews and approves the official filing. Not legal advice.</div>
  `
  return `<!doctype html><html><head><meta charset="utf-8"><style>${FORM_CSS}</style></head><body>${body}</body></html>`
}

export async function downloadFormPdf(data, c) {
  const o = await org()
  return ellis.exportPdf({ html: formHtml(data, c, o), suggestedName: `${c.applicantName} - ${data.formName}`.replace(/[^\w -]/g, '') })
}

// Save the filled form straight to the Desktop (used by Ask Ellis commands).
export async function downloadFormPdfToDesktop(data, c) {
  const o = await org()
  return ellis.exportPdfToDesktop({ html: formHtml(data, c, o), suggestedName: `${c.applicantName} - ${data.formName}`.replace(/[^\w -]/g, '') })
}

function translationHtml(data, docName, c, orgName) {
  const para = (t) => esc(t).split('\n').map((l) => l.trim() ? `<p style="font-size:13px;line-height:1.7;margin:0 0 6px">${l}</p>` : '').join('')
  const body = `
    <h1>Certified Translation</h1>
    <div class="sub">${esc(docName || 'Document')} &nbsp; <span class="chip">${esc(data.sourceLanguage)} → ${esc(data.targetLanguage)}</span> ${data.certified ? '<span class="chip">Certified</span>' : ''}</div>
    <div class="sec">Case</div>
    <table>${rows([
      { label: 'Applicant', value: c.applicantName },
      { label: 'Route', value: `${c.originCountry || '—'} → ${c.destinationCountry}` }
    ])}</table>
    <div class="sec">${esc(data.targetLanguage)} translation</div>
    ${para(data.translation)}
    ${(data.glossary || []).length ? `<div class="sec">Glossary</div><table>${(data.glossary).map((g) => `<tr><td class="k">${esc(g.term)}</td><td>${esc(g.meaning)}</td></tr>`).join('')}</table>` : ''}
    ${data.note ? `<p style="font-size:12px;color:#555;margin-top:18px">${esc(data.note)}</p>` : ''}
  `
  return shell('Certified Translation', orgName, body)
}

export async function downloadTranslationPdf(data, docName, c) {
  const o = await org()
  return ellis.exportPdf({ html: translationHtml(data, docName, c, o), suggestedName: `${c.applicantName} - Translation`.replace(/[^\w -]/g, '') })
}
export async function downloadTranslationPdfToDesktop(data, docName, c) {
  const o = await org()
  return ellis.exportPdfToDesktop({ html: translationHtml(data, docName, c, o), suggestedName: `${c.applicantName} - Translation`.replace(/[^\w -]/g, '') })
}

function handoffHtml(data, c, orgName) {
  const exhibits = `<table>${(data.exhibits || []).map((e) => `<tr><td class="k">${esc(e.label)}</td><td>${esc(e.description)} <span class="chip">${esc(e.status)}</span></td></tr>`).join('')}</table>`
  const body = `
    <h1>${esc(data.title)}</h1>
    <div class="sub">${esc(data.overview)} &nbsp; <span class="chip">${esc(data.recommendedPathway || c.visaType || c.pathway)}</span></div>
    <div class="sec">Case</div>
    <table>${rows([
      { label: 'Applicant', value: c.applicantName },
      { label: 'Route', value: `${c.originCountry || '—'} → ${c.destinationCountry}` },
      { label: 'Pathway', value: c.visaType || c.pathway },
      { label: 'Employer', value: c.employer || 'n/a' }
    ])}</table>
    <div class="sec">Exhibits</div>${exhibits}
    ${(data.openItems || []).length ? `<div class="sec">Open items for counsel</div>${list(data.openItems)}` : ''}
    ${data.attorneyNotes ? `<div class="sec">Attorney notes</div><p style="font-size:13px;line-height:1.6">${esc(data.attorneyNotes)}</p>` : ''}
  `
  return shell('Counsel handoff', orgName, body)
}

export async function downloadHandoffPdf(data, c) {
  const o = await org()
  return ellis.exportPdf({ html: handoffHtml(data, c, o), suggestedName: `${c.applicantName} - Counsel Handoff`.replace(/[^\w -]/g, '') })
}
export async function downloadHandoffPdfToDesktop(data, c) {
  const o = await org()
  return ellis.exportPdfToDesktop({ html: handoffHtml(data, c, o), suggestedName: `${c.applicantName} - Counsel Handoff`.replace(/[^\w -]/g, '') })
}
